...
prometheus.exporter.unix "encaged" {
...
discovery.docker "encaged_containers" {
  host = "unix:///var/run/docker.sock"
}

// Relabel docker containers for logs
discovery.relabel "encaged_docker_logs" {
  targets = discovery.docker.encaged_containers.targets
  ...
  rule {
    target_label = "job"
    replacement = "docker/encaged"
  }
}

// Collect Docker container logs
loki.source.docker "encaged_containers" {
  host             = "unix:///var/run/docker.sock"
  targets          = discovery.relabel.encaged_docker_logs.output
  labels           = { platform = "docker", host = "encaged" }
